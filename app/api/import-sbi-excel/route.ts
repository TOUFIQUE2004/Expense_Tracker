...
export async function POST(req: Request) {
    try {
        ...
        for (const row of rows) {
            ...
            // Note: classification is calculated but not stored until schema is updated
            const classification = classifyTransaction(desc);

            // Add to database - use clerkUserId since that's what the schema references
            const tx = await db.transaction.create({
                data: {
                    userId: user.clerkUserId, // Use clerkUserId to match the schema relationship
                    text: desc,
                    amount,
